...
const updateAlbumSchema = zod_1.z.object({
    name: zod_1.z.string().min(1, "Album name is required").optional(),
    description: zod_1.z.string().optional(),
...
albums.post("/create", requireAuth, async (c) => {
    try {
        ...
        const imagePromises = imageFiles.map(async (file, index) => {
            ...
            try {
                const publicUrl = await supabase_1.storageHelpers.uploadFile("albums", filePath, file);
        ...
        if (albumData.imageIdsToDelete?.length > 0) {
            ...
            for (const imageId of albumData.imageIdsToDelete) {
                ...
                if (image) {
                    ...
                    if (filePath) {
                        await supabase_1.storageHelpers.deleteFile("albums", `${id}/${filePath}`);
                    }
                }
            }
...
albums.delete("/:albumId/images/:imageId", requireAuth, async (c) => {
    try {
        ...
        const filePath = image.url.split("/").pop();
        if (filePath) {
            await supabase_1.storageHelpers.deleteFile("albums", `${albumId}/${filePath}`);
        }
